...
export default function FarmerProfile() {
  ...
  const handleDeleteFarmer = async () => {
    if (
      !window.confirm(
        `Delete ${farmer.fullName}? This cannot be undone.`
      )
    ) {
      return;
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F6F2E9] px-6">
        <div className="max-w-sm w-full bg-white/50 backdrop-blur-sm rounded-3xl border border-[#F3C6C6]/50 shadow-sm p-8 text-center flex flex-col items-center">
          ...
          <h3 className="text-[#C24949] text-base font-semibold mb-1">
            Unable to load
          </h3>

          <p className="text-[#C24949]/80 text-sm font-medium">
            {error}
          </p>
  ...
  const totalBill = services.reduce(
    (sum, service) =>
      sum + Number(service.totalBill || 0),
    0
  );

  const totalCollected = services.reduce(
    (sum, service) =>
      sum + Number(service.amountPaid || 0),
    0
  );

  const totalPending = services.reduce(
    (sum, service) =>
      sum + Number(service.pendingAmount || 0),
    0
  );
      ...
      <div className="px-5 -mt-16 relative z-10 max-w-md mx-auto space-y-4">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-black/[0.04] p-5">
          <div className="flex justify-between items-start mb-4">
            <div className="flex gap-4 items-center">
              ...
              <div>
                <h1 className="text-xl font-bold text-[#1F2A22] leading-tight mb-1">
                  {farmer.fullName}
                </h1>
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div className="bg-[#F6F2E9]/50 rounded-xl p-3 border border-[#1F3D2B]/5">
              <p className="text-[10px] uppercase font-bold text-[#1F2A22]/50 tracking-wider mb-1">
                Village
              </p>
          ...
          <div className="pt-4 border-t border-black/[0.05]">
            <p className="text-[10px] uppercase font-bold text-[#1F2A22]/40 tracking-widest mb-3">
              Payment Summary
            </p>
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <div className="bg-[#F6F2E9]/70 rounded-xl p-3 border border-black/[0.04]">
                <p className="text-[9px] uppercase font-bold text-[#1F2A22]/45 tracking-wide mb-1">
                  Total Bill
                </p>

                <p className="text-sm font-black text-[#1F2A22] break-words">
              ...
              <div className="bg-[#E9F3E9]/70 rounded-xl p-3 border border-[#4C9A5A]/10">
                <p className="text-[9px] uppercase font-bold text-[#4C9A5A]/70 tracking-wide mb-1">
                  Collected
                </p>

                <p className="text-sm font-black text-[#4C9A5A] break-words">
              ...
              <div className="bg-[#FEF3C7]/50 rounded-xl p-3 border border-[#D97706]/10">
                <p className="text-[9px] uppercase font-bold text-[#D97706]/70 tracking-wide mb-1">
                  Pending
                </p>

                <p className="text-sm font-black text-[#D97706] break-words">
          ...
          {deleteError && (
            <div className="bg-[#FCEDED] border border-[#F3C6C6] rounded-xl p-3 mt-4 flex items-center gap-2">
              <svg
                viewBox="0 0 24 24"
                className="h-4 w-4 text-[#C24949] shrink-0"
                fill="none"
              >
                <circle
                  cx="12"
        ...
        <div className="mt-8">
          <div className="flex items-center justify-between mb-3 px-1">
            <h2 className="text-lg font-bold text-[#1F2A22] tracking-tight">
              Service History
            </h2>
